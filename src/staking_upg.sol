 ...
 */
contract TokenStaking is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function data() public view returns(bytes memory){
        bytes memory data = abi.encodeWithSignature("initialize(address,uint256,address,address)", token, claimInterval, nft, ifacenft);   
