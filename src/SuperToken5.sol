...
// Import Libraries
import "./libraries/FeeCalculator2.sol"; // Updated FeeCalculator
...
contract SuperToken5 is ReentrancyGuard, AccessControl {
    ...
    using FeeCalculator2 for LoanTerms; // Ensures LoanTerms can call calculateTimeBasedFee
    ...
    /**function borrow(
        ...
        // Ensure the borrower is a depositor
        require(
            lpToken.balanceOf(msg.sender) > 0,
            "SuperToken5: must be a depositor to borrow"
        );
        ...
        EFItoken.burnFrom(_msgSender(), gasFee); // Collect gas fee
    ...
    */
    ...
    function repay() external nonReentrant {
        ...
        uint256 fee = calculateFee(loan.amount, loan.loanType, loan.borrowBlock, currentBlock);
    ...
    {
        LoanTerms memory terms = loanTerms[loanType];
        fee = terms.calculateTimeBasedFee(
            amount,
            borrowBlock,
            currentBlock,
            BLOCKS_IN_A_YEAR,
