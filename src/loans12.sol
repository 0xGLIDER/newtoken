...
contract MergedStablecoinLending is AccessControl, ReentrancyGuard {
    ...
    uint256 public LOAN_DURATION_IN_BLOCKS = 241920; // Approx. 1 year (assuming 15 sec blocks)

    // Pool Metrics
    uint256 public totalDeposits;        // Total stablecoins deposited
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (totalShares == 0) {
            ...
            // Mint LP tokens proportional to the deposit relative to available liquidity
            sharesToMint = (amount * totalShares) / availableLiquidity;
        }
     ...
     * Users receive a proportional share of the pool's available liquidity.
    ...
    function withdraw(uint256 sharesToBurn) external nonReentrant {
        ...
        uint256 amountToWithdraw = (sharesToBurn * availableLiquidity) / totalShares;

        // Burn the shares from the user
        depositShares.burnFrom(_msgSender(), sharesToBurn);

        // Transfer the stablecoin amount corresponding to the shares burned
